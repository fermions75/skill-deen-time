...
```bash
curl -sL "https://api.aladhan.com/v1/timings/{DD-MM-YYYY}?latitude={LAT}&longitude={LNG}&method={METHOD}"
```
...
```bash
curl -sL "https://api.aladhan.com/v1/calendarByCity/{YEAR}/{MONTH}?city={CITY}&country={COUNTRY}&method={METHOD}"
```
...
→ Call: `curl -sL "https://api.aladhan.com/v1/timingsByCity?city=London&country=United%20Kingdom&method=3"`
→ Display all prayer times in a formatted table with both Gregorian and Hijri dates.

**User**: "When is Iftar in Dubai?"
→ Call: `curl -sL "https://api.aladhan.com/v1/timingsByCity?city=Dubai&country=United%20Arab%20Emirates&method=4"`
...
→ Call: `curl -sL "https://api.aladhan.com/v1/calendarByCity/2026/2?city=Istanbul&country=Turkey&method=13"` (and March if Ramadan spans two months)
→ Filter to only Ramadan days (check Hijri month = 9 / Ramadan) and present the Suhoor/Iftar table.

**User**: "Suhoor time for New York?"
→ Call: `curl -sL "https://api.aladhan.com/v1/timingsByCity?city=New%20York&country=United%20States&method=2"`
...
→ Call: `curl -sL "https://api.aladhan.com/v1/timings/19-02-2026?latitude=21.4225&longitude=39.8262&method=4"`
